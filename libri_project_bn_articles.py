...
for sheet in bn_magazines_spreadsheets:
    ...
    bn_magazines = pd.concat([bn_magazines, df])
...
x245 = []
for i, e in tqdm(zip(multiple_poems['001'].to_list(), multiple_poems['245'].to_list()), total=len(multiple_poems['001'].to_list())):
    test_dict = marc_parser_to_dict(e, '\$')
    data = ' '.join([test_dict.get(e).replace(' /', '') for e in test_dict if e in ['$a', '$b']]).split(' ; ')
    data = [(i, '10$a' + e + ' /$c' + test_dict.get('$c') if '$c' in test_dict else '10$a' + e) for e in data]
    x245.extend(data)
 
field_245 = pd.DataFrame(x245, columns=['001', '245'])

multiple_poems = pd.merge(field_245, multiple_poems.drop(columns='245'), on='001')

x700 = dict(zip(multiple_poems.index.to_list(), multiple_poems['700'].to_list()))
...
x700 = {k:remove_wrong_700(v) for k,v in x700.items()}.values()

multiple_poems['700'] = x700
...
#errors
with open(f'data/libri_bn_articles_errors_{year}-{month}-{day}.txt', encoding='utf8') as file:
